...
unsigned int CPU_fetchCode(char *mem, int codeOffset){
    // Read the 32-bit word from the simulated memory at the given offset
    unsigned int instruction = read_dword(mem, codeOffset);
    return instruction;
}
...
unsigned char CPU_Decode(unsigned int machineCode){
    // Shift right by 26 and mask to isolate the top 6 bits
...
void printRegisterFiles(){
    puts("\n---- Register Files ----");
    // Iterate through the 32 general-purpose registers
    for(int i = 0; i < 32; i++){
        // Print the register name and its current value in Hex
        printf("%s\t= 0x%08X   ", regNameTab[i], regFile[i]);
        
        // Change line every 4 registers to make a grid
        if ((i + 1) % 4 == 0) {
            printf("\n");
        }
    }
...
void printDataMemoryDump(char *mem){
    ...
    // Dump 256 bytes starting at the DATASECTION offset (0x2000)
